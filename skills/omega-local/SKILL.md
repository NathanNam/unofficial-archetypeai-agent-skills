...
3. Concatenate embeddings across sensors at the **window** level for the downstream model (joint state). The canonical code form of this pattern — joint-state concat + optional L2 / standardization / PCA into `(X, y, metadata)` arrays — lives in [`atai-newton-omega-model-data-prep`](../atai-newton-omega-model-data-prep/SKILL.md)'s `FeaturePreparer`.
...
Pattern adapted from [`archetypeai/omega-1-4-preflight`](https://github.com/archetypeai/omega-1-4-preflight) (that repo targets the cloud `machine-state-job-pipeline` for `omega_1_4_base`; the static checks transfer cleanly to local 1.3 inference). For the *active cleanup* counterpart — same diagnostics but with gap-aware blocking + imputation that produces a cleaned dataframe instead of just a report — see [`atai-newton-omega-model-data-prep`](../atai-newton-omega-model-data-prep/SKILL.md)'s `DataPreprocessor`. Run once after loading, before any transformations:
